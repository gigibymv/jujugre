...
export function saveErrorLogEntries(entries: ErrorLogEntry[]): void {
  if (typeof window === 'undefined') return;
...
export function appendErrorLogEntry(input: NewErrorLogInput): ErrorLogEntry {
  const now = new Date();
